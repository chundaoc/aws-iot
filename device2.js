...
device
  .on('connect', function() {
    ...
    device.publish('topic/test', JSON.stringify({ device: 2 }));
    });
...
// Thing Shadow named 'device2'.
//
    thingShadows.register( 'device2' );
//
// 2 seconds after registering, update the Thing Shadow named 
// 'device2' with the latest device state and save the clientToken
...
//
    setTimeout( function() {
       clientTokenUpdate = thingShadows.update('device2', rgbLedLampState  );
       }, 2000 );
    });
